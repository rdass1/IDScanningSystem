        ...
        if len(line) > 0 and line.startswith("AB",0,2) and line[2:].isdigit() and len(line[2:]) == 10:
            ...
            if (output != None):
                if(output['active']==False):
                    ...
                    command1 = "UPDATE memberInfo.activeMember SET active = FALS' WHERE uid = '%s';" % (output['uid'])
                # print(output)
                print(output['uid'])
                print(command1)
                cur.execute(command1)
            # else:
            #     command1 = "INSERT INTO memberInfo.activeMember (uid,id,active,building) VALUES ('%s','%s','%s','%s');"
