...
while True:
    ...
    try:
        ...
        if len(line) > 0 and line.startswith("AB",0,2) and line[2:].isdigit() and len(line[2:]) == 10:
            ...
            command = "SELECT * FROM memberInfo.activeMember WHERE uid = '%s';" % (currentUser['uid'])
            cur.execute(command)
            output = cur.fetchone()
            print(currentUser)
            if (output != None):
                ...
                command = "INSERT INTO memberInfo.activeMember (uid,active,building) VALUES (%s,%s,%s,%s);"
                cur.execute("""INSERT INTO memberInfo.activeMember (uid,active,building) VALUES (%s,%s,%s,%s);""",((currentUser['uid']),("True"),(scanner["building"])))
