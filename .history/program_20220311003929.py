...
while True:
    ...
    try:
        ...
        if len(line) > 0 and line.startswith("AB",0,2) and line[2:].isdigit() and len(line[2:]) == 10:
            ...
            cur.execute("SELECT * FROM memberInfo.activeMember WHERE uid = '%s'",currentUser[""])
            output =
            if (output != None):
                ...
                # cur.execute("INSERT INTO memberInfo.activeMember (uid,id,active,building) VALUES ('%s','%s','%s','%s');",(output['uid'],output['id'],'True',scanner["building"]))
                print("added")
            cur.close()
