        ...
        if len(line) > 0 and line.startswith("AB",0,2) and line[2:].isdigit() and len(line[2:]) == 10:
            ...
            if (output != None):
                if(output['active']==False):
                    command2= "UPDATE memberInfo.activeMember SET active = TRUE WHERE uid = '%s';" % (output['uid'])
                else:
                    command2 = "UPDATE memberInfo.activeMember SET active = FALSE WHERE uid = '%s';" % (output['uid'])
                # print(output)
                print(output['uid'])
                print(command1)
